...
/**
 * Creates a new schema with empty table in the database upon SQL query that can then be populated with data.
 * If schema or table already exist does nothing.
 * The methods in this class all throw a DAOException if the connection to the database fails.
 * @author Daria Bogush
 */
 ...
 */
    public final void createDatabase() throws DAOException {
        ...
        try {
            ...
            throw new DAOException("Failed to connect to database while create database.", e);
        } finally {
            try {
                if (statement != null) {
                    statement.close();
                ...
                throw new DAOException("Failed to close connection while create database.");
            }
        }
    }
